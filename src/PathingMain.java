...
public class PathingMain extends PApplet
{
   ...
   private boolean dfs(Point pos, GridValues[][] grid, List<Point> path)
   {
      ...
      Point[] neighbors = {new Point(pos.x-1,pos.y), new Point(pos.x+1,pos.y),
              new Point(pos.x,pos.y-1), new Point(pos.x,pos.y+1)};

      for (Point nextPt : neighbors) {
         if (withinBounds(nextPt, grid) &&
                 grid[nextPt.y][nextPt.x] != GridValues.OBSTACLE &&
                 grid[nextPt.y][nextPt.x] != GridValues.SEARCHED) {
            if (dfs(nextPt, grid, path)) {
               path.add(nextPt);
               return true;
            }
         }
