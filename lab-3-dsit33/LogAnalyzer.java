...
public class LogAnalyzer
{
   ...
   private static void processViewEntry(final String[] words, final Map<String, List<View>> viewsFromSession)
   {
	...
	views.add(View(words[VIEW_PRODUCT_ID]));
      ...
      )
   {
   	...
   	purchases.add(Buy(words[BUY_PRODUCT_ID], (int)words[BUY_PRICE]*(int)words[BUY_QUANTITY]));
   }
      ...
      )
   {
      ...
      for (String session : viewsFromSession.keySet()){
      	...
      	avg = total/(viewsFromSession.get(session).size());
      	for (Buy buy : buysFromSession.get(session)){
      		System.out.println("\t%s ".format(buy.getProductID) + "%s".format(buy.getProductPrice - avg));
      	}
      }


      /*for (String session : buysFromSession.keySet()){
      	System.out.println("%s".format(session));
      	List<String> purchases = buysFromSession.get(session);
      	for (int i = 1; i < purchases.size(); i++){
      		int max = max(purchases.get(i-1).getTotalSpent(),purchases.get(i).getTotalSpent());
      	}
      	for (Buy buy : purchases){
      		if (buy.getTotalSpent() < max){
      			System.out.println("%s ".format(buy.getProductID()) + "%s".format(buy.getTotalSpent()-max));
      		}
      	}
      }*/
      /* add printing */
      ...
      )
   {
      ...
      //print customer name
      //get any one purchase from the customer
      //check to see how many sessions the same item was viewed in by the same customer
      for (String customer : sessionsFromCustomer.keySet()){
      	...
      	for (String session : sessionsFromCustomer.get(customer)){
      		for (Buy buy : buysFromSession.get(session)){
      			...
      			while (i == 0){
      				for (View view : viewsFromSession.get(session)){
      					if (buy.getProductID() == view.getProductID()){
      						views += 1;
      						i++;
      					}
      				}
      	...
      	System.out.println("%s ".format(buy.getProductID()) + views);
      }

      /*for (String customer : sessionsFromCustomer.keySet()){
      	System.out.println("%s".format(customer));
      	List<String> sessions = sessionsFromCustomer.get(customer);
      	for (String session : sessions){
      		List views = viewsFromSession.get(session);
      		for (View view : views){
      			int total = 0;
      			for (j = 0; j < views.size(); j++) {
      			String trigger = view.getProductID();
      			if (trigger == views.get(i).getProductID()){
      				total += 1;
      				}
 c      		}
      ...
      )
   {
   	...
   	for (String customer : sessionsFromCustomer.keySet()){
   		for (String session : sessionsFromCustomer.get(customer)){
   			total += viewsFromSession.get(session).size();
   			total -= buysFromSession.get(session).size();
   			num++;
   		}
   	}
   	avg = total/num;
   	System.out.println("Average Views without Purchase: " + avg);
      printSessionPriceDifference(sessionsFromCustomer, buysFromSession, viewsFromSession);
      printCustomerItemViewsForPurchase(sessionsFromCustomer, buysFromSession, viewsFromSession);
